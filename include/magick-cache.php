...
<h5>Get content from the MagickCache</h5>
<p>Eventually you will want retrieve your content, let's get our original cast image from the cache:</p>
<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</samp></pre></ul>
...
<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt -extract 100x100+0+0 get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</samp></pre></ul>
<p>To resize instead, do not specify the offset:</p>

<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt -extract 100x100 get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</samp></pre></ul>

<p>If your image is scrambled, provide the passphrase to descramble it:</p>

<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt -passphrase passphrase.txt get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</samp></pre></ul>

<h5>Delete content from the MagickCache</h5>
<p>We can explicitly delete content:</p>
<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt delete /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson</samp></pre></ul>
<p>or we can delete all cast images that have expired (exceeded their respective time to live), try this comand:</p>
<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt expire /opt/magick-cache movies/image/mission-impossible/cast</samp></pre></ul>
<h5>Identify the MagickCache content</h5>
<p>Perhaps you want to identify all the content you own:</p>
<ul><pre class="bg-light text-dark"><samp>$ magick-cache -passkey passkey.txt identify /opt/magick-cache movies/image/mission-impossible/cast
