...
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt -ttl "2 days" put /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson 0200508-rebecca-ferguson.jpg</code></pre></ul>
<p>Where <code>cast-key.txt</code> contains your passphrase.  Don't forget your cache key.  Without it, you will not be able to get, list, delete or expire your content.</p>
...
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt -extract 100x100+0+0 get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</code></pre></ul>
<p>To resize instead, do not specify the offset:</p>

<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt -extract 100x100 get /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson rebecca-ferguson.png</code></pre></ul>

<h5>Delete content from the Magick Cache</h5>
<p>We can explicitedly delete content:</p>
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt delete /opt/magick-cache movies/image/mission-impossible/cast/rebecca-ferguson</code></pre></ul>
<p>or we can delete cast images that have expired (exceeded their respective time to live), try this comand:</p>
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt expire /opt/magick-cache movies/image/mission-impossible/cast</code></pre></ul>
<h5>List the Magick Cache content</h5>
<p>Perhaps you want to audit all the content you own:</p>
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt list /opt/magick-cache movies/image/mission-impossible/cast
...
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt put /opt/magick-cache movies/blob/mission-impossible/cast/rebecca-ferguson 0200508-rebecca-ferguson.mp4</code></pre></ul>
<p> or</p>
<ul><pre class="bg-light text-dark"><code>$ magick-cache -key cast-key.txt put /opt/magick-cache movies/meta/mission-impossible/cast/rebecca-ferguson 0200508-rebecca-ferguson.txt</code></pre></ul>
