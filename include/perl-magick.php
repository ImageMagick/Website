...
<pre class="bg-light text-dark mx-4"><samp>yum install ImageMagick-perl
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>tar xvzf ImageMagick.tar.gz
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>sudo make install
</samp></pre>

<p>You may need to configure the dynamic linker run-time bindings:</p>

<pre class="bg-light text-dark mx-4"><samp>sudo ldconfig /usr/local/lib
</samp></pre>


<p>Finally, verify the PerlMagick install worked properly, type</p>

<pre class="bg-light text-dark mx-4"><samp>perl -MImage::Magick -le 'print Image::Magick->QuantumDepth'
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>cd PerlMagick
perl Makefile.nt
nmake
nmake install
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>make test
</samp></pre>

<p>Use the 'prove' utilize to execute a test from the build folder:</p>
...
<pre class="bg-light text-dark mx-4"><samp>cd demo
make
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>use Image::Magick;
...
<pre class="bg-light text-dark mx-4"><samp>use Image::Magick::Q16;
...
<pre class="bg-light text-dark mx-4"><samp>$image = Image::Magick-&gt;new(size=&gt;'384x256');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$ENV{PATH}' . "='/../bin:/usr/bin:/usr/local/bin';
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>#!/usr/local/bin/perl
use Image::Magick;<br />
my($image, $x);<br />
$image = Image::Magick-&gt;new;
...
<pre class="pre-scrollable bg-light text-dark mx-4"><samp>#!/usr/local/bin/perl
use Image::Magick;<br />
my($image, $p, $q);<br />
...
<pre class="bg-light text-dark mx-4"><samp>$image = Image::Magick-&gt;new;
...
<pre class="bg-light text-dark mx-4"><samp>@pixels = $image-&gt;GetPixel(x=&gt;1,y=&gt;1);
$pixels[0]*=0.5;
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Quantize(colorspace=&gt;'gray');
</samp></pre>

<p>Let's annotate an image with a Taipai TrueType font:</p>

<pre class="bg-light text-dark mx-4"><samp>$text = 'Works like magick!';
$image-&gt;Annotate(font=&gt;'kai.ttf', pointsize=&gt;40, fill=&gt;'green', text=&gt;$text);
</samp></pre>

<p>Perhaps you want to extract all the pixel intensities from an image and write them to STDOUT:</p>

<pre class="bg-light text-dark mx-4"><samp>@pixels = $image-&gt;GetPixels(map=&gt;'I', height=&gt;$height, width=&gt;$width, normalize=&gt;true);
binmode STDOUT;
...
<pre class="bg-light text-dark mx-4"><samp>$i = $#$p"+1";   # return the number of images associated with object p
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Write(filename=&gt;'image.png', compression=&gt;'None');
</samp></pre>

<p>Use <samp>-</samp> as the filename to method Read() to read from standard in or to method Write() to write to standard out:</p>

<pre class="bg-light text-dark mx-4"><samp>binmode STDOUT;
$image-&gt;Write('png:-');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$filename = "image.png";
open(IMAGE, ">$filename");
...
<pre class="bg-light text-dark mx-4"><samp>image%03d.miff
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Crop(geometry=&gt;'100x100+10+20');
...
<pre class="bg-light text-dark mx-4"><samp>image-&gt;Draw(fill=&gt;'red', primitive=&gt;'rectangle',
 points=&gt;'20,20 100,100  40,40 200,200  60,60 300,300');
</samp></pre>

<p>Is equivalent to</p>

<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Draw(fill=&gt;'red', primitive=&gt;'@draw.txt');
</samp></pre>

<p>Where <samp>draw.txt</samp> is a file on disk that contains this:</p>

<pre class="bg-light text-dark mx-4"><samp>rectangle 20, 20 100, 100
...
<pre class="pre-scrollable bg-light text-dark mx-4"><samp>%b   file size
%c   comment
%d   directory
...
<pre class="bg-light text-dark mx-4"><samp>text=&gt;"%m:%f %wx%h"
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Set(dither=&gt;'True');
$image-&gt;[$x]-&gt;Set(delay=&gt;3);
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>($a, $b, $c) = $image-&gt;Get('colorspace', 'magick', 'adjoin');
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Montage(geometry=&gt;'160x160', tile=&gt;'2x2', texture=&gt;'granite:');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>@blobs = $image-&gt;ImageToBlob();
...
<pre class="bg-light text-dark mx-4"><samp>@blob=$db-&gt;GetImage();
...
<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Append(stack=&gt;{true,false});
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$q = $p-&gt;Clone();
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>@features = $image-&gt;Features(1);
</samp></pre>

<p>The Flatten() method flattens a set of images and returns it. For example,</p>

<pre class="bg-light text-dark mx-4"><samp>$p = $images-&gt;Flatten(background=&gt;'none');
$p-&gt;Write('flatten.png');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Fx(expression=&gt;'(g+b)/2.0',channel=&gt;'red');
$p-&gt;Write('fx.miff');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Morph(frames=&gt;<i>integer</i>);
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;Crop('340x256+0+0');
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;MogrifyRegion('100x100+40+50', 'modulate', brightness=&gt;50);
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>@phash = $image-&gt;PerceptualHash();</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>($width, $height, $size, $format) = $image-&gt;Ping('logo.png');
...
<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Poly([0.5,1.0,0.25,2.0,1.0,1.0]);
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$preview = $image-&gt;Preview('Gamma');
$preview-&gt;Display();
</samp></pre>

<p>To have full control over text positioning you need font metric information. Use</p>

<pre class="bg-light text-dark mx-4"><samp>($x_ppem, $y_ppem, $ascender, $descender, $width, $height, $max_advance) =
...
<pre class="bg-light text-dark mx-4"><samp>@colors = $image-&gt;QueryColor();
...
<pre class="bg-light text-dark mx-4"><samp>$name = $image-&gt;QueryColorname('rgba(80,60,0,0)');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>@fonts = $image-&gt;QueryFont();
...
<pre class="bg-light text-dark mx-4"><samp>@formats = $image-&gt;QueryFormat();
($adjoin, $blob_support, $raw, $decoder, $encoder, $description, $module) =
  $image-&gt;QueryFormat('gif');
</samp></pre>

<p>Call MagickToMime() with the image format name to get its MIME type such as <samp>image/tiff</samp> from <samp>tif</samp>.</p>

<pre class="bg-light text-dark mx-4"><samp>$mime = $image-&gt;MagickToMime('tif');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$image-&gt;RemoteCommand('image.jpg');
</samp></pre>

<p>The Smush() method smushes a set of images together. For example,</p>

<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Smush(stack=&gt;{true,false},offset=&gt;<var>integer</var>);
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>@statistics = $image-&gt;Statistics();
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$p = $image-&gt;Transform(crop=&gt;'100x100+0+0');
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$x = $image-&gt;Read(...);
...
<pre class="bg-light text-dark mx-4"><samp>$x = $image-&gt;Crop(...);
warn "$x" if "$x";      # print the error message
$x =~ /(\d+)/;
print $1;               # print the error number
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$x = $image-&gt;Montage(...);
...
<pre class="bg-light text-dark mx-4"><samp>Error 400: Memory allocation failed
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>$x = $image-&gt;Read('rose.png');
$x =~ /(\d+)/;
...
<pre class="pre-scrollable bg-light text-dark mx-4"><samp>BlobError
BlobWarning
CacheError
CacheWarning
CoderError
...
<pre class="bg-light text-dark mx-4"><samp>Image::Magick-&gt;QuantumDepth
</samp></pre>
