...
<pre class="bg-light text-dark mx-4"><samp>magick stream -map rgb -storage-type char image.jpg pixels.dat
...
<pre class="bg-light text-dark mx-4"><samp>magick stream -map i -storage-type double -extract 100x100+30+40 image.tif gray.raw
</samp></pre>

<p>You can also associate the region to extract with the image filename:</p>

<pre class="bg-light text-dark mx-4"><samp>magick stream -map i -storage-type double 'image.tif[100x100+30+40]' gray.raw
</samp></pre>
