...
<p>Congratulations, you have a working ImageMagick distribution and you are ready to use ImageMagick to <a href="https://usage.imagemagick.org/">convert, compose, or edit</a> your images or perhaps you'll want to use one of the <a href="/develop/">Application Program Interfaces</a> for C, C++, Perl, and others.</p>
...
<p>Next, launch your Visual Studio IDE and choose <samp>Open->Project</samp>.  Select the configure workspace from the <samp>ImageMagick-<?php echo(MagickVersion); ?>/VisualMagick/configure</samp> folder and open configure.sln.  Choose <samp>Build->Build Solution</samp>
...
<p>Press <samp>Next</samp> and click on the multi-threaded static build.  Now press, on <samp>Next</samp> twice and finally <samp>Finish</samp>.  The configuration utility just created a workspace required to build ImageMagick from source.  Choose <samp>Open->Project</samp> and select the VisualStaticMT workspace from the <samp>ImageMagick-<?php echo(MagickVersion); ?>/VisualMagick/</samp>  folder.  Finally, choose <samp>Build->Build Solution</samp> to compile and build the ImageMagick distribution.</p>
...
<p>The above instructions will satisfy a great number of ImageMagick users, but we suspect a few will have additional questions or problems to consider.  For example, what does one do if ImageMagick fails to configure or compile?  Or what if you want to install ImageMagick in a place other than the <samp>ImageMagick-<?php echo(MagickVersion); ?>/VisualMagick/bin</samp> folder?  You will find the answer to these questions, and more, in <a href="/advanced-windows-installation/">Advanced Windows Source Installation</a>.</p>
